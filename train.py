...
parser = argparse.ArgumentParser(description='')#用来导入解析数据
...
args = parser.parse_args() #用来解析上面定义的命令行参数
EPS = 1e-12 #EPS函数用于保证log函数里面的参数大于零
...
def save(saver, sess, logdir, step): #保存模型的save函数
   ...
   if not os.path.exists(logdir): #如果路径不存在即创建
      os.makedirs(logdir)
   saver.save(sess, checkpoint_path, global_step=step) #保存模型（sees代表当前变量，checkpoint_path代表保存路径，global_step：表示当前是第几步）
   print('The checkpoint has been created.')#给出已经创建保存点
 
def cv_inv_proc(img): #cv_inv_proc函数将读取图片时归一化的图片还原成原图
    img_rgb = (img + 1.) * 127.5
    return img_rgb.astype(np.float32) #返回rgb格式的图像，方便cv2写图像
...
def main(): #训练程序的主函数
    ...
    dis_fake = discriminator(image=train_picture, targets=gen_label, df_dim=64, reuse=tf.AUTO_REUSE, name="discriminator") #判别器返回的对生成(虚假的)标签判别结果
    ...
    saver = tf.train.Saver(var_list=tf.global_variables(), max_to_keep=50) #模型保存器(保存50个，前面save函数的参数)
